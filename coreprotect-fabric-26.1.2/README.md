# CoreProtect Fabric（Minecraft 26.1.2 版）

一个以 **CoreProtect** 插件为蓝本制作的 Fabric 服务器模组（Minecraft 26.1.2）：方块/容器/击杀/聊天/命令/会话全记录，支持回滚（Rollback）、恢复（Restore）、查询（Lookup）、检查模式（Inspect）与多语言切换。

> 本目录为 **Minecraft 26.1.2** 专用构建。
