...
**CoreProtect Fabric** 是一款面向 Minecraft 26.1.2 Fabric 服务器的方块级操作记录与回滚模组，以 Bukkit/Spigot 生态中久负盛名的 **CoreProtect** 插件为蓝本独立实现，将"记录一切、可查可回滚"的服务器治理能力带入 Fabric 生态。方块、容器、漏斗、掉落物、告示牌编辑、实体、聊天、命令、在线会话九大类行为全量记录，并提供回滚（Rollback）、恢复（Restore）、查询（Lookup）、检查模式（Inspect）、在线记录查询与多语言切换，输出配色与 CoreProtect 同款。
...
|---|---|
| Minecraft | **26.1.2** |
| 服务端 | Fabric Loader ≥ 0.19 + Fabric API |
| Java | 25 |
| 安装 | 将 `coreprotect-fabric-26.1.2-1.8.2.jar` 放入 `mods/` 即可，首次启动自动生成配置与数据库 |
