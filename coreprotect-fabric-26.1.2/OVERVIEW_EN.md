# CoreProtect Fabric — Mod Overview (v1.8.2)

## One-Line Summary

**CoreProtect Fabric** is a block-level logging and rollback mod for Minecraft 26.1.2 Fabric servers. Independently implemented with the renowned Bukkit/Spigot plugin **CoreProtect** as its blueprint, it brings "log everything, query and roll back anything" server governance to the Fabric ecosystem. It records blocks, containers, hoppers, item drops, sign edits, entities, chat, commands, and online sessions, and ships with rollback, restore, lookup, inspection mode, online-record queries, and multi-language support — with the same output color scheme as CoreProtect.
...
|---|---|
| Minecraft | **26.1.2** |
| Server | Fabric Loader ≥ 0.19 + Fabric API |
| Java | 25 |
| Install | Drop `coreprotect-fabric-26.1.2-1.8.2.jar` into `mods/`; config and database are generated on first launch |
