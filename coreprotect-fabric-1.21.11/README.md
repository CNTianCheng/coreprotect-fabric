# CoreProtect Fabric（Minecraft 1.21.11 版）

一个以 **CoreProtect** 插件为蓝本制作的 Fabric 服务器模组（Minecraft 1.21.11）：方块/容器/击杀/聊天/命令/会话全记录，支持回滚（Rollback）、恢复（Restore）、查询（Lookup）、检查模式（Inspect）与多语言切换。

> 本目录为 **Minecraft 1.21.11** 专用构建。
...
2. 将 `coreprotect-fabric-1.21.11-1.8.2.jar` 放入服务端 `mods/` 文件夹。
...
# 需要 JDK 21
./gradlew build
# 产物位于 build/libs/coreprotect-fabric-1.21.11-1.8.2.jar
```
