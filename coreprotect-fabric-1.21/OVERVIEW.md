# CoreProtect Fabric 模组概述（v1.7.2）

## 一句话介绍

**CoreProtect Fabric** 是一款面向 Minecraft 1.21 Fabric 服务器的方块级操作记录与回滚模组，以 Bukkit/Spigot 生态中久负盛名的 **CoreProtect** 插件为蓝本独立实现，将"记录一切、可查可回滚"的服务器治理能力带入 Fabric 生态。方块、容器、漏斗、掉落物、告示牌编辑、实体、聊天、命令、在线会话九大类行为全量记录，并提供回滚（Rollback）、恢复（Restore）、查询（Lookup）、检查模式（Inspect）、在线记录查询与多语言切换，输出配色与 CoreProtect 同款。
...
| 项目 | 要求 |
|---|---|
| Minecraft | **1.21**（1.21.1 请使用 `coreprotect-fabric/` 目录的对应构建） |
| 服务端 | Fabric Loader ≥ 0.16 + Fabric API |
| Java | 21 |
| 安装 | 将 `coreprotect-fabric-1.21-1.7.2.jar` 放入 `mods/` 即可，首次启动自动生成配置与数据库 |
