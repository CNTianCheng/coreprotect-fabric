# CoreProtect Fabric v1.8.2 (Minecraft 1.21.11)

A block-level logging and rollback mod for **Fabric servers**, independently implemented with the renowned [CoreProtect](https://github.com/PlayPro/CoreProtect) plugin as its blueprint. This is the **v1.8.2 bug-fix release** for the Minecraft 1.21.11 build; the same fixes ship for 1.21 and 26.1.2.
...
- Drop `coreprotect-fabric-1.21.11-1.8.2.jar` into the server's `mods/` folder; config and database are created on first launch
...
- 将 `coreprotect-fabric-1.21.11-1.8.2.jar` 放入 `mods/`，首次启动自动生成配置与数据库
