# CoreProtect Fabric v1.8.1 (Minecraft 26.1.2)

A block-level logging and rollback mod for **Fabric servers**, independently implemented with the renowned [CoreProtect](https://github.com/PlayPro/CoreProtect) plugin as its blueprint. This release is the **Minecraft 26.1.2 build**. Other Minecraft versions (1.21.1 / 1.21.2~1.21.11 / 26.1~26.2) are available in their own releases.

---

## English

> **This build brings Minecraft 26.1.2 to full feature parity with the 1.21 flagship build (v1.8.1)**: identical commands, logging coverage, colors, database schema v2 with crash/power-loss protection, bStats metrics and update checks.
...
- Drop `coreprotect-fabric-26.1.2-1.8.1.jar` into the server's `mods/` folder; config and database are created on first launch
...
> **本构建使 Minecraft 26.1.2 与 1.21 旗舰版 v1.8.1 功能完全一致**：命令、记录范围、输出配色、v2 数据库结构与崩溃/断电防护、bStats 统计和更新检查全部相同。
...
- 要求：Minecraft **26.1.2**、Java **25**、Fabric Loader ≥ 0.19 + Fabric API
- 将 `coreprotect-fabric-26.1.2-1.8.1.jar` 放入 `mods/`，首次启动自动生成配置与数据库
